...
import { addToCart, decrementCartItem, incrementCartItem, removeCartItem, setCartItemQuantity } from '../store/posSlice';
import { PosProductSearchBar } from '../components/PosProductSearchBar';
import { ProductGridCard } from '../components/ProductGridCard';
import { CartItemRow } from '../components/CartItemRow';
import { SaleSummary } from '../components/SaleSummary';
import { calculateDiscountAmount, calculateSubtotal, calculateTotal, isCartValid } from '../utils/saleMath';
...
export default function CreateSalePage() {
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((product) =>
      [product.name, product.manufacturer, product.category].filter(Boolean).some((field) => field!.toLowerCase().includes(trimmed))
    );
  }, [products, query]);
  ...
  const renderProductGrid = () => {
    ...
    if (status === 'failed') {
      return <ErrorState message={error ?? 'Product search failed.'} onRetry={() => dispatch(fetchProducts())} />;
    }
  ...
  return (
    <Content>
      <PageHeader title="New Sale" subtitle="Search products and build the cart" onBack={() => navigate('/sales')} />

      <SplitLayout>
        <SearchColumn>
          <PosProductSearchBar ref={searchInputRef} value={query} onChange={setQuery} />
          {renderProductGrid()}
        </SearchColumn>

        <CartColumn>
          <CartTitle>Current Sale</CartTitle>
          {cart.length === 0 ? (
            <EmptyState
            ...
            <>
              ...
              <SummaryBlock>
                <SaleSummary subtotal={subtotal} discountAmount={discountAmount} total={total} />
                {hasStockIssue && (
                  <ValidationNotice role="alert">
                    ⚠ Resolve insufficient stock before proceeding to checkout.
                  </ValidationNotice>
                )}
                <Button type="button" $fullWidth disabled={!cartValid} onClick={() => navigate('/sales/checkout')}>
                  Proceed to Checkout
                </Button>
              </SummaryBlock>
            </>
